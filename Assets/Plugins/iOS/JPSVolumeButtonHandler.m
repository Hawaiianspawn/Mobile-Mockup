...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if (context == sessionContext) {
        ...
        if (newVolume > oldVolume) {
            if (self.upBlock){
                self.upBlock();
                UnitySendMessage("Game", "volumeUp", "Fuck off");
            }
        } else {
            if (self.downBlock){
                 self.downBlock();
                UnitySendMessage("Game", "volumeDown", "Fuck On");}
        }
...
- (void)callUnityObject:(const char*)object Method:(const char*)method Parameter:(const float*)parameter {
